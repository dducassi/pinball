    ...
    def _register_tables(self):
        ...
        self.tables.append(Table(
            ...
            sounds={
                ...
                'flipper': 'flipper.wav',
                'launch': 'launch.ogg',
                'ball_lost': 'ball_lost.ogg',
                'ball_saved': 'ball_save.ogg',
                'block_hit': 'block_sound.wav',
                'game_over': 'game_over_sound.ogg'
            }
        ...
        self.tables.append(Table(
            ...
            tagline="START GAME",
            music_path=os.path.join(self.base_dir, 'minimalmusic.ogg'),
            bg_path=None,
            lane_bg_path=None,
        ...
        self.tables.append(Table(
            name="FAERIE CIRCLE",
            builder_class=FairyTableBuilder,
            title="FAERIE CIRCLE",
            tagline="ENTER THE MOON REALM",
            music_path=os.path.join(self.base_dir, 'fairymusic.ogg'),  # or custom music
            bg_path=os.path.join(self.base_dir, 'fairy_circle.png'),
            lane_bg_path=os.path.join(self.base_dir, 'lane_bg.png'),  # reuse or custom
            use_ball_image=True,
            large_bumper_messages={
                self.settings.wht: "MOONLIGHT!",
                self.settings.pnk: "PIXIE DUST!",
                self.settings.ppl: "FAERIE FLUTTER!"
            },
            small_bumper_messages=["TWINKLE!", "GLOW!", "SPARKLE!"],
            color_cycle=[self.settings.wht, self.settings.pnk, self.settings.ppl],
            tint_background=True,
            pph=100,
            sounds={
                'bumper': 'fairybumper.ogg',
                'flipper': 'flipper.wav',
                'launch': 'launch.ogg',
                'ball_lost': 'ball_lost.ogg',
                'ball_saved': 'ball_save.ogg',
...
class FairyTableBuilder(TableBuilderBase):
    def __init__(self, settings, images):
        super().__init__(settings)
        self.settings = settings
        # Use default textures (same as wizard, but we may ignore some)
        self.block_texture = images.get('block_texture')
        self.tri_texture = images.get('tri_texture')
        self.tri_flipped = images.get('tri_flipped')
        self.tri_mirrored = images.get('tri_mirrored')
    ...
    def generate_bumpers(self, images):
        bumpers = []
        lights = []
        # Use custom moon image if provided, else fallback to orb_image
        moon_image = images.get('moon_image') or images.get('orb_image')
        light_image = images.get('light_image')

        # Single large moon bumper at top center
        x = self.playfield_width / 2
        y = self.playfield_height / 6 + self.top_margin + self.settings.br
        bump_rad = 9.9 * self.settings.br   # large bumper
        color = self.settings.wht
        ...
        y = self.top_margin + self.playfield_height // 2 + self.settings.br
        bump_rad = 1  * self.settings.br
        bumpers.append(Bumper(x, y, self.settings.wht, bump_rad, moon_image))
        lights.append(Light(x, y, self.settings.br * 1.2, color, light_image))

        # Upper right rock
        x = 5/7 * self.playfield_width - self.settings.br
        y = self.top_margin + self.playfield_height // 2 + self.settings.br
        bump_rad = 1  * self.settings.br
        bumpers.append(Bumper(x, y, self.settings.wht, bump_rad, moon_image))
        lights.append(Light(x, y, self.settings.br * 1.2, color, light_image))
    ...
    def generate_top_guide(self):
        """Same concave guide as wizard (needed for lane)."""
        wall_thick = self.settings.lane_wall_thickness
        lane_left = self.playfield_width + 1
        lane_right = self.screen_width - 4
        guide_height = 40
    ...
    def generate_blocks(self):
        ...
        blocks.append(Block(right_funnel_vertices, self.settings.slv, self.settings.restitution, pattern=True))
         ...
         # Top guide (concave)
        blocks.append(self.generate_top_guide())
        ...
        # One-way wall to prevent re-entry
        blocks.append(self.generate_one_way_wall())

        # Top border (below title area)
        top_y = self.top_margin
        blocks.append(Block(
            [(0, top_y), (0, top_y + wall_thick), (self.screen_width, top_y + wall_thick), (self.screen_width, top_y)],
            (100,100,100), restitution=self.settings.restitution, image=self.edge_horz
        ))

        # Left border
        blocks.append(Block(
            [(0, 0), (0, self.screen_height), (wall_thick, self.screen_height), (wall_thick, 0)],
            (100,100,100), restitution=self.settings.restitution, image=self.edge_vert
        ))
        # Right border
        blocks.append(Block(
            [(self.screen_width - wall_thick, 0), (self.screen_width - wall_thick, self.screen_height),
             (self.screen_width, self.screen_height), (self.screen_width, 0)],
            (100,100,100), restitution=self.settings.restitution, image=self.edge_vert
        ))
       
        # Bottom drain
        blocks.append(self.generate_bottom_wall())
        ...
        # Left lane wall (shortened)
        blocks.append(Block(
            [(lane_left - wall_thick, lane_bottom - 5), (lane_left - wall_thick, lane_top + self.guide_height * 2),
             (lane_left, lane_top + self.guide_height * 2), (lane_left, lane_bottom - 5)],
            (100,100,100), restitution=self.settings.restitution, image=self.edge_vert
        ))
        # Right lane wall (inner)
        blocks.append(Block(
            [(lane_left, lane_bottom - 5), (lane_left, lane_top + self.guide_height * 2),
             (lane_left + wall_thick, lane_top + self.guide_height * 2), (lane_left + wall_thick, lane_bottom - 5)],
            (100,100,100), restitution=self.settings.restitution, image=self.edge_vert
        ))
        # Lane cap
        blocks.append(Block(
            [(lane_left - 7, lane_top + self.guide_height * 2), (lane_left + 7, lane_top + self.guide_height * 2),
             (lane_left + 7, lane_top + self.guide_height * 2 + 5), (lane_left - 7, lane_top + self.guide_height * 2 + 5)],
            (100,100,100), restitution=self.settings.restitution, image=self.edge_horz
        ))
